...
(`FilteringNioEndpoint`) which rejects connections from blocked IPs at
...
`blockedIpsFile` attribute to automatically load blocked IPs from a file at startup:
...
The blocked IPs file is monitored for changes; if you modify the file while Tomcat is running, the changes will be automatically detected and reloaded without requiring a restart.
